...
export class MercatorTransform implements ProjectionTransform {
    ...
    getVisibleUnwrappedCoordinates(tileID: CanonicalTileID): UnwrappedTileID[] {
        ...
        if (this._transform._renderWorldCopies) {
            const utl = this.screenPointToMercatorCoordinate(new Point(0, 0));
            const utr = this.screenPointToMercatorCoordinate(new Point(this._transform._width, 0));
            const ubl = this.screenPointToMercatorCoordinate(new Point(this._transform._width, this._transform._height));
            const ubr = this.screenPointToMercatorCoordinate(new Point(0, this._transform._height));
    ...
    recalculateZoomAndCenter(terrain?: Terrain): void {
        ...
        const center = this.screenPointToLocation(this._transform.centerPoint, terrain);
    ...
    getBounds(): LngLatBounds {
        const top = Math.max(0, this._transform._height / 2 - getMercatorHorizon(this._transform));
        return new LngLatBounds()
            .extend(this.screenPointToLocation(new Point(0, top)))
            .extend(this.screenPointToLocation(new Point(this._transform._width, top)))
            .extend(this.screenPointToLocation(new Point(this._transform._width, this._transform._height)))
            .extend(this.screenPointToLocation(new Point(0, this._transform._height)));
    }
    ...
    defaultConstrain: TransformConstrainFunction = (lngLat, zoom) => {
        ...
        let lngRange = this._transform._lngRange;

        if (!this._transform._renderWorldCopies && lngRange === null) {
        ...
        let scaleY = 0;
        let scaleX = 0;
        const {x: screenWidth, y: screenHeight} = this._transform.size;

        if (this._transform._latRange) {
            const latRange = this._transform._latRange;
    ...
    _calculateNearFarZ(cameraToSeaLevelDistance: number, limitedPitchRadians: number, offset: Point): void {
        ...
        const cameraToLowestPointDistance = cameraToSeaLevelDistance - minElevation * this._transform._pixelPerMeter / Math.cos(limitedPitchRadians);
    ...
    calcMatrices(nearFarZ: NearFarZ | null = this._transform.nearFarZOverride): void {
        ...
        m[8] = -offset.x * 2 / this._transform._width;
        m[9] = offset.y * 2 / this._transform._height;
        ...
        mat4.scale(m, m, [1, 1, this._transform._pixelPerMeter]);
        ...
        mat4.scale(this._fogMatrix, this._fogMatrix, [1, 1, this._transform._pixelPerMeter]);
        ...
        const xShift = (this._transform._width % 2) / 2, yShift = (this._transform._height % 2) / 2,
