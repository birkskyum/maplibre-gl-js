...
export class MercatorTransform implements ITransform {
    ...
    /**
     * Returns the segment of the ray through the given screen pixel from the clip space depth `nearDepth`
     * to the far plane. The default depth 0 lies at about twice the near plane distance; -1 is the near plane.
     */
    private getRaySegmentFromPixel(p: Point, nearDepth: number = 0): RaySegment {
        const coord0 = [p.x, p.y, nearDepth, 1] as vec4;
