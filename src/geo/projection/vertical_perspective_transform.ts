...
export class VerticalPerspectiveTransform implements ProjectionTransform {
    ...
    public transformLightDirection(dir: vec3): vec3 {
        const sphereX = this._transform._center.lng * Math.PI / 180.0;
        const sphereY = this._transform._center.lat * Math.PI / 180.0;
    ...
    public getPixelScale(): number {
        return 1.0 / Math.cos(this._transform._center.lat * Math.PI / 180);
    }

    public getCircleRadiusCorrection(): number {
        return Math.cos(this._transform._center.lat * Math.PI / 180);
    }
    ...
    calcMatrices(): void {
        ...
        globeMatrix[8] = -offset.x * 2 / this._transform._width;
        globeMatrix[9] = offset.y * 2 / this._transform._height;
    ...
    defaultConstrain: TransformConstrainFunction = (lngLat, zoom) => {
        ...
        const constrainedZoom = clamp(+zoom, this._transform.minZoom + getZoomAdjustment(0, constrainedLat), this._transform.maxZoom);
        return {
            center: new LngLat(
                lngLat.lng,
                constrainedLat
    ...
    locationToScreenPoint(lnglat: LngLat, terrain?: Terrain): Point {
        ...
        if (terrain) {
            const elevation = terrain.getElevationForLngLatZoom(lnglat, this._transform._tileZoom);
